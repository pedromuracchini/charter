...
from chokepoint.errors import ConfigurationWarning
...
def test_enforce_mode_blocks():
    interceptor = ChokepointInterceptor(policies=[_blocking_policy()], mode="enforce")
    with pytest.raises(GuardBlocked):
        interceptor.call("t", lambda: {"ok": True})


def test_dry_run_mode_never_blocks_but_records():
    interceptor = ChokepointInterceptor(policies=[_blocking_policy()], mode="dry_run")
...
def test_observe_mode_never_blocks_but_records():
    interceptor = ChokepointInterceptor(policies=[_blocking_policy()], mode="observe")
    result = interceptor.call("t", lambda: {"ok": True})
    assert result == {"ok": True}
    events = ActionLedger.current().events()
...
def test_otel_tracer_is_honored_without_configure_otel():
    ...
    from chokepoint.otel.config import otel_available, reset_otel
    ...
    interceptor = ChokepointInterceptor(policies=[_blocking_policy()], mode="observe", otel_tracer=provider)
    interceptor.call("t", lambda: {"ok": True})
...
def test_step_counters_evict_oldest_session_beyond_max_sessions():
    interceptor = ChokepointInterceptor(policies=[], max_sessions=2)
...
def test_per_interceptor_ledgers_stay_separate_from_each_other_and_the_global_one():
    ...
    a = ChokepointInterceptor(policies=[_blocking_policy()], mode="observe", ledger=ledger_a)
    b = ChokepointInterceptor(policies=[_blocking_policy()], mode="observe", ledger=ledger_b)
...
def test_interceptor_ledger_records_tool_errors_too():
    ledger = ActionLedger()
    interceptor = ChokepointInterceptor(policies=[], ledger=ledger)
...
def test_interceptor_redactor_is_what_scrubs_the_recorded_args():
    ...
    interceptor = ChokepointInterceptor(
...
async def test_interceptor_redactor_and_ledger_apply_to_acall_too():
    ledger = ActionLedger()
    interceptor = ChokepointInterceptor(policies=[], ledger=ledger, redactor=_KeyErasingRedactor())
