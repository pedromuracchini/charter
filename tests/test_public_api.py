...
#: A frozen snapshot of everything `import chokepoint` promises. Adding a name
...
#: Every sub-package that re-exports something. `from chokepoint.ledger import
#: ActionLedger` used to fail outright: these files were all 0 bytes.
SUBPACKAGES = [
    "chokepoint.core",
    "chokepoint.ledger",
    "chokepoint.escalation",
    "chokepoint.report",
    "chokepoint.linter",
    "chokepoint.testing",
    "chokepoint.multiagent",
    "chokepoint.otel",
    "chokepoint.cli",
    "chokepoint.policies",
]
...
def test_every_name_in_all_actually_exists():
    missing = [name for name in chokepoint.__all__ if not hasattr(chokepoint, name)]
    assert missing == []
...
def test_public_api_matches_the_snapshot():
    ...
    """
    assert list(chokepoint.__all__) == PUBLIC_API


@pytest.mark.parametrize("name", sorted(PINNED_SIGNATURES))
def test_entry_point_signatures_are_stable(name):
    assert signature_shape(getattr(chokepoint, name)) == PINNED_SIGNATURES[name]
...
def test_interceptor_call_keeps_tool_name_and_func_positional_only():
    ...
    parameters = inspect.signature(chokepoint.ChokepointInterceptor.call).parameters
...
def test_previously_unexported_names_are_reachable(name):
    assert hasattr(chokepoint, name)


def test_importing_chokepoint_does_not_pull_in_a_framework_sdk():
    """Adapters import their framework lazily, inside `applies_to()`, so
    `import chokepoint` stays cheap and works with none of the extras installed.
...
def test_version_is_exposed():
    assert isinstance(chokepoint.__version__, str)
    assert chokepoint.__version__
