...
This page explains *why* Chokepoint is shaped the way it is. For signature-level
...
control. Chokepoint expresses those rules as ordinary Python predicates evaluated
...
Chokepoint is **middleware, not a harness**. It does not run a plan→act→observe
...
places a guarded call actually runs. `@guard`, `ChokepointInterceptor.call()` and
`ChokepointInterceptor.acall()` are thin call-sites: they assemble a
...
same caller. Outside any interceptor, `chokepoint.session(...)` sets it
explicitly.
...
and Chokepoint warns when it detects the collision rather than silently dropping
the value.
...
`chokepoint.escalation` and are deliberately *not* auto-registered: there is no
...
Chokepoint's answer is to share the tool but never share an interceptor without
...
dynamic agents, one shared `ChokepointRegistry` replaces per-interceptor wiring.
...
silently defeating any role-scoped policy. `chokepoint lint` reports that as an
error.
...
omission. No span is emitted for it, because Chokepoint decided nothing there —
...
answer is a real sandbox with a real kernel boundary; Chokepoint is the
