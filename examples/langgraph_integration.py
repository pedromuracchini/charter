...
used through `create_react_agent`/`ToolNode`) with Tollgate before handing
them to LangGraph.
...
def main() -> None:
    ...
    interceptor = TollgateInterceptor(policies=[large_transfer_policy])
    ...
    print(asyncio.run(wrapped_tools[1].ainvoke({"query": "tollgate"})))
