...
- The version of `chokepoint` affected (`python -c "import chokepoint; print(chokepoint.__version__)"`).

We aim to acknowledge reports within a few business days.

## Trust boundaries to be aware of

Chokepoint is an authorization layer, not a sandbox. A few things worth being
explicit about:

- **The CLI executes arbitrary code.** `chokepoint report`, `chokepoint lint`,
  `chokepoint replay --agent <file>`, and `chokepoint repl --agent <file>` all
...
- **Policy predicates run with the same privileges as your agent process.**
  ...
  callables evaluated in-process — they are not sandboxed. Chokepoint protects
...
- **The default `EscalationHandler` denies by design.** With no handler
  ...
  applies: make sure it authenticates its approval source — Chokepoint has no
...
- **Tool arguments are redacted before they are recorded, but redaction is
  ...
  message (see `chokepoint.redaction`). PII patterns are opt-in via
  `configure_redaction(include_pii=True)`. This is pattern matching, not
  classification: a credential in a shape Chokepoint doesn't recognise, or an
