...
def run_repl(policies: list[Policy]) -> None:
    ...
    `quit`/EOF."""
    print("tollgate policy REPL — type a tool name, or 'quit' to exit.")
    while True:
        try:
        ...
        if tool_name in ("quit", "exit", ""):
            break
        try:
            raw_args = input("args (JSON)> ").strip() or "{}"
        ...
        for result in trace.results:
            ...
            print(f"  [{mark}] {result.policy_name}: {result.reason}")
        if trace.decision is None:
            print("decision: ALLOW (no rule fired)")
        else:
            print(f"decision: {trace.decision.on_fail.value.upper()} — {trace.decision.reason}")
