...
place. Registered by default (see `tollgate.adapters`); `applies_to()` only
attempts the optional `langchain_core` import lazily, inside the method body,
so `import tollgate` never depends on it being installed.
"""
...
def _wrap_tool(tool: BaseTool, interceptor: TollgateInterceptor) -> BaseTool:
