...
tools *before* constructing the `Agent`:
    ...
    wrapped = tollgate.wrap(my_tools, interceptor)
...
`FileSearchTool`, ...) are passed through unchanged — Tollgate can't
meaningfully guard tool execution it doesn't control. Registered by default
(see `tollgate.adapters`); `applies_to()` only attempts the optional `agents`
...
run loop. Tollgate is complementary, not a replacement: it gives you the same
...
def _wrap_tool(tool: Any, interceptor: TollgateInterceptor) -> Any:
