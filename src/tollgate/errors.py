"""Tollgate's exception and warning hierarchy.

Every error the framework raises on purpose derives from `TollgateError`, so a
...
Warnings are separate. Tollgate warns rather than raises for configurations
...
class TollgateError(Exception):
    """Base class for every error Tollgate raises deliberately."""


class ConfigurationError(TollgateError, ValueError):
...
class EscalationError(TollgateError, RuntimeError):
...
class LedgerError(TollgateError):
...
class AdapterError(TollgateError, TypeError):
    """No adapter could wrap the given agent or tool object."""


class TollgateWarning(UserWarning):
    """Base class for every warning Tollgate issues."""


class ConfigurationWarning(TollgateWarning):
