"""The core evaluation pipeline shared by `@guard` and `CharterInterceptor`.
...
from charter.core.escalation import resolve_handler
...
from charter.multiagent.delegation import delegation_depth
from charter.otel.metrics import record_decision, record_delegation_depth, record_escalation
from charter.otel.spans import evaluate_span, should_sample
from charter.redaction import Redactor, current_redactor

logger = logging.getLogger("charter.engine")
_undo_logger = logging.getLogger("charter.reversible")
_escalation_logger = logging.getLogger("charter.escalation")
...
def _run_with_timeout(fn: Callable[[], bool | Awaitable[bool]], timeout_s: float) -> bool:
    ...
    # Copy the caller's context so a handler reading `charter.current_scope()`
    ...
    if inspect.isawaitable(result):
        _escalation_logger.error(
            ...
            "(CharterInterceptor.call() or @guard on a sync tool) — use acall()/an "
...
async def _run_with_timeout_async(
    ...
    reads `charter.current_scope()` sees the caller's identity rather than the
    root scope.
    """
    try:
...
def _record_tool_error(
    ...
    *,
    ...
    No OTEL span is emitted: Charter made no decision here, and whatever
...
def evaluate_call(
    *,
    ...
    provider for this call's spans (see `CharterInterceptor.otel_tracer`).
    """
    ...
    if scope.call_state is not None:
        ...
        # deciding on. Counts attempts, not successes — see `charter.state`.
