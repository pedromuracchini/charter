...
Every module under `charter.otel` degrades to a no-op when the `opentelemetry`
packages aren't installed (the `charter[otel]` extra) or when `configure_otel`
...
def configure_otel(
    ...
    """Point charter at an OTEL tracer provider and configure event sampling.
...
def reset_otel() -> None:
    ...
    from charter.otel.metrics import reset_instruments
