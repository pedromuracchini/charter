"""Span emission for each Charter evaluation.

Emits a `charter.evaluate` child span per rule evaluation, carrying the
...
from charter.decisions import ALLOW, BLOCK, GuardDecision
from charter.otel.config import current_settings, otel_available
...
def evaluate_span(
    ...
    """Emit a `charter.evaluate` span for one rule evaluation.

    `tracer_provider`, if given (typically `CharterInterceptor.otel_tracer`),
    ...
    tracer = trace.get_tracer("charter", tracer_provider=provider)
    with tracer.start_as_current_span("charter.evaluate") as span:
        # `charter.tool` is what makes these spans groupable in a backend —
        # "which tool is getting blocked?" is the first question anyone asks.
        span.set_attribute("charter.tool", ctx.tool_name)
        span.set_attribute("charter.policy", decision.policy_name or "")
        span.set_attribute("charter.policy_hash", decision.policy_hash or "")
        span.set_attribute("charter.action", decision.action.value)
        span.set_attribute("charter.hook", hook)
        span.set_attribute("charter.severity", decision.severity)
        span.set_attribute("charter.reason", decision.reason)
        span.set_attribute("charter.latency_ms", latency_ms)
        span.set_attribute("charter.session_id", ctx.session_id)
        span.set_attribute("charter.step_index", ctx.step_index)
        span.set_attribute("charter.caller_agent_id", ctx.caller_agent_id or "")
        span.set_attribute("charter.caller_role", ctx.caller_role or "")
        span.set_attribute("charter.trust_level", ctx.trust_level)
        span.set_attribute("charter.delegation_chain", "→".join(ctx.delegation_chain))
        span.set_attribute("charter.dry_run", dry_run)
