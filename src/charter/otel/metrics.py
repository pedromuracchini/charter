"""Metric emission for each Charter decision.
...
`charter.report.policy_report` instead of pushed as live OTEL gauges — sliding
...
from charter.decisions import BLOCK, GuardDecision
...
def _meter() -> Any | None:
    ...
    return metrics.get_meter("charter")
...
def record_decision(
    ...
    _counter("charter.decisions_total", meter).add(1, attrs)
    _histogram("charter.policy_latency_ms", meter).record(latency_ms, attrs)
    if decision.undo_executed:
        _counter("charter.undo_total", meter).add(1, attrs)
    if cross_agent and decision.action is BLOCK:
        _counter("charter.cross_agent_blocks_total", meter).add(1, attrs)
...
def record_escalation(
    ...
    *,
    ...
    operational signals Charter can emit — and there was previously no metric
    ...
    _counter("charter.escalations_total", meter).add(1, attrs)
    _histogram("charter.escalation_latency_ms", meter).record(latency_ms, attrs)
...
def record_delegation_depth(depth: int, *, agent_id: str | None = None) -> None:
    ...
    if meter is None:
        return
    _histogram("charter.delegation_depth", meter).record(depth, {"agent": agent_id or ""})
