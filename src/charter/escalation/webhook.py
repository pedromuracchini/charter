...
`charter.escalation.slack`). Uses only `urllib.request` — no new dependency.
"""
...
logger = logging.getLogger("charter.escalation.webhook")
...
class WebhookEscalationHandler(EscalationHandler):
    ...
    secret, a bearer token, ...) — per `SECURITY.md`, Charter has no opinion
