...
`EscalationHandler` implementation under `charter.escalation`.

Arguments are redacted before they go out. An escalation message is the least
controlled destination Charter writes to — it lands in a Slack channel with
its own membership, or at a webhook endpoint on someone else's infrastructure
— so it gets the same scrubbing the ledger does. See `charter.redaction`.
...
from charter.redaction import current_redactor
