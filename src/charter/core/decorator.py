...
`charter.session(...)`) — `dry_run`/`observe` modes and agent identity are a
`CharterInterceptor` concern.
...
from charter.decisions import Decision, Severity
...
class _GuardDecorator(Protocol):
    ...
    Charter ships `py.typed`, so a decorator that erased types would make
...
def guard(
    ...
    *,
    ...
    def decorator(func: Callable[..., Any] | ReversibleAction) -> Callable[..., Any]:
        ...
        wrapper.__charter_policy__ = policy  # type: ignore[union-attr]
        wrapper.__charter_reversible__ = reversible  # type: ignore[union-attr]
        wrapper.__charter_tool_name__ = tool_name  # type: ignore[union-attr]
        if isinstance(func, ReversibleAction):
