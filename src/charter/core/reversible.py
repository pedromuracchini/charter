...
`charter lint` warns about a `"high"` action with no `escalate_to`.
"""
...
from charter.decisions import BLOCK, ESCALATE, RuleResult
...
class ReversibleAction:
    ...
    `CharterInterceptor.acall()` detect an async `do_fn` automatically (see
    ...
    `CharterInterceptor.call()`) returns an un-awaited coroutine, which is a
