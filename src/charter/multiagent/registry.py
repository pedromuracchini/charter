"""`CharterRegistry` — central registry of agent identities and roles.

Used by `CharterInterceptor` to populate `ctx.caller_role` / `ctx.trust_level`
...
class CharterRegistry:
    ...
    One registry shared by several `CharterInterceptor`s is the alternative
    ...
    def __repr__(self) -> str:
        ...
        return f"<CharterRegistry agents={len(self._identities)}[{agents}]>"
    ...
    def register(
        ...
        Args:
            ...
            delegation_chain: This agent's **ancestors only**, ordered
                ...
                path — see `CharterInterceptor._self_inclusive_chain`.
