...
Charter records `ctx.args` verbatim for every decision, and those records go
...
often the point of the call — so enabling them is a deliberate choice:
    ...
    charter.configure_redaction(include_pii=True, keys=["ssn", "dob"])

Turn it off entirely with `configure_redaction(enabled=False)`.

One consequence worth knowing: `charter.replay()` reconstructs its context
...
class Redactor(Protocol):
    ...
    DLP client, say) can satisfy it without inheriting from Charter.
    """
...
def configure_redaction(
    *,
    ...
    recorded are not revisited.
        ...
        charter.configure_redaction(include_pii=True, keys=["ssn", "mrn"])
