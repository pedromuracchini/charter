...
`ActionLedger.configure(sink_path=...)` (exported as `charter.configure_ledger`).
...
from charter.decisions import RuleResult, pick_decision
from charter.errors import ConfigurationError, LedgerEventNotFound
from charter.ledger.event import LedgerEvent
from charter.redaction import contains_placeholder

logger = logging.getLogger("charter.ledger")
...
class ActionLedger:
    ...
    def export_policy_graph(self, format: GraphFormat = "mermaid") -> str:
        from charter.report.graph import policy_graph
...
class ReplayResult:
    ...
    #: as unreliable. See `charter.redaction`.
    redacted: bool = False
