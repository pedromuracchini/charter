...
`FileSearchTool`, ...) are passed through unchanged — Charter can't
meaningfully guard tool execution it doesn't control. Registered by default
(see `charter.adapters`); `applies_to()` only attempts the optional `agents`
...
run loop. Charter is complementary, not a replacement: it gives you the same
...
def _wrap_tool(tool: Any, interceptor: CharterInterceptor) -> Any:
