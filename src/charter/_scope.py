...
from somewhere else. `ExecutionScope` is that somewhere: `CharterInterceptor` (or
...
class ExecutionScope:
    ...
    #: see zero rather than stale numbers. See `charter.state`.
    call_state: CallState | None = None


_ROOT_SCOPE = ExecutionScope()
_current_scope: ContextVar[ExecutionScope] = ContextVar("charter_scope", default=_ROOT_SCOPE)
...
def use_scope(scope: ExecutionScope) -> Iterator[ExecutionScope]:
    ...
    Used by `CharterInterceptor` so that any `@guard`-decorated helper called
...
def session(**overrides: Any) -> Iterator[ExecutionScope]:
    ...
    `CharterInterceptor` — the interceptor manages scope on its own.
    """
