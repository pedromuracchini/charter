"""The core evaluation pipeline shared by `@guard` and `ChokepointInterceptor`.
...
from chokepoint.core.escalation import resolve_handler
from chokepoint.core.policy_set import Hook, Policy
...
from chokepoint.multiagent.delegation import delegation_depth
from chokepoint.otel.metrics import record_decision, record_delegation_depth, record_escalation
from chokepoint.otel.spans import evaluate_span, should_sample
from chokepoint.redaction import Redactor, current_redactor

logger = logging.getLogger("chokepoint.engine")
_undo_logger = logging.getLogger("chokepoint.reversible")
_escalation_logger = logging.getLogger("chokepoint.escalation")
...
def _run_with_timeout(fn: Callable[[], bool | Awaitable[bool]], timeout_s: float) -> bool:
    ...
    # Copy the caller's context so a handler reading `chokepoint.current_scope()`
    ...
    if inspect.isawaitable(result):
        _escalation_logger.error(
            ...
            "(ChokepointInterceptor.call() or @guard on a sync tool) — use acall()/an "
...
async def _run_with_timeout_async(
    ...
    reads `chokepoint.current_scope()` sees the caller's identity rather than the
    root scope.
    """
    try:
...
def _record_tool_error(
    ...
    *,
    ...
    No OTEL span is emitted: Chokepoint made no decision here, and whatever
...
def evaluate_call(
    *,
    ...
    provider for this call's spans (see `ChokepointInterceptor.otel_tracer`).
    """
    ...
    if scope.call_state is not None:
        ...
        # deciding on. Counts attempts, not successes — see `chokepoint.state`.
