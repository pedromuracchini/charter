"""Span emission for each Chokepoint evaluation.

Emits a `chokepoint.evaluate` child span per rule evaluation, carrying the
...
from chokepoint.decisions import ALLOW, BLOCK, GuardDecision
from chokepoint.otel.config import current_settings, otel_available
...
def evaluate_span(
    ...
    """Emit a `chokepoint.evaluate` span for one rule evaluation.

    `tracer_provider`, if given (typically `ChokepointInterceptor.otel_tracer`),
    ...
    tracer = trace.get_tracer("chokepoint", tracer_provider=provider)
    with tracer.start_as_current_span("chokepoint.evaluate") as span:
        # `chokepoint.tool` is what makes these spans groupable in a backend —
        # "which tool is getting blocked?" is the first question anyone asks.
        span.set_attribute("chokepoint.tool", ctx.tool_name)
        span.set_attribute("chokepoint.policy", decision.policy_name or "")
        span.set_attribute("chokepoint.policy_hash", decision.policy_hash or "")
        span.set_attribute("chokepoint.action", decision.action.value)
        span.set_attribute("chokepoint.hook", hook)
        span.set_attribute("chokepoint.severity", decision.severity)
        span.set_attribute("chokepoint.reason", decision.reason)
        span.set_attribute("chokepoint.latency_ms", latency_ms)
        span.set_attribute("chokepoint.session_id", ctx.session_id)
        span.set_attribute("chokepoint.step_index", ctx.step_index)
        span.set_attribute("chokepoint.caller_agent_id", ctx.caller_agent_id or "")
        span.set_attribute("chokepoint.caller_role", ctx.caller_role or "")
        span.set_attribute("chokepoint.trust_level", ctx.trust_level)
        span.set_attribute("chokepoint.delegation_chain", "→".join(ctx.delegation_chain))
        span.set_attribute("chokepoint.dry_run", dry_run)
