"""Metric emission for each Chokepoint decision.
...
`chokepoint.report.policy_report` instead of pushed as live OTEL gauges — sliding
...
from chokepoint.decisions import BLOCK, GuardDecision
...
def _meter() -> Any | None:
    ...
    return metrics.get_meter("chokepoint")
...
def record_decision(
    ...
    _counter("chokepoint.decisions_total", meter).add(1, attrs)
    _histogram("chokepoint.policy_latency_ms", meter).record(latency_ms, attrs)
    if decision.undo_executed:
        _counter("chokepoint.undo_total", meter).add(1, attrs)
    if cross_agent and decision.action is BLOCK:
        _counter("chokepoint.cross_agent_blocks_total", meter).add(1, attrs)
...
def record_escalation(
    ...
    *,
    ...
    operational signals Chokepoint can emit — and there was previously no metric
    ...
    _counter("chokepoint.escalations_total", meter).add(1, attrs)
    _histogram("chokepoint.escalation_latency_ms", meter).record(latency_ms, attrs)
...
def record_delegation_depth(depth: int, *, agent_id: str | None = None) -> None:
    ...
    if meter is None:
        return
    _histogram("chokepoint.delegation_depth", meter).record(depth, {"agent": agent_id or ""})
