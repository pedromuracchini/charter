...
Every module under `chokepoint.otel` degrades to a no-op when the `opentelemetry`
packages aren't installed (the `chokepoint[otel]` extra) or when `configure_otel`
...
def configure_otel(
    ...
    """Point chokepoint at an OTEL tracer provider and configure event sampling.
...
def reset_otel() -> None:
    ...
    from chokepoint.otel.metrics import reset_instruments
