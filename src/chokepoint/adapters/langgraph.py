...
place. Registered by default (see `chokepoint.adapters`); `applies_to()` only
attempts the optional `langchain_core` import lazily, inside the method body,
so `import chokepoint` never depends on it being installed.
"""
...
def _wrap_tool(tool: BaseTool, interceptor: ChokepointInterceptor) -> BaseTool:
