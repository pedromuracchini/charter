...
vantage points. Chokepoint supports both, because they answer different
questions:
...
(see `chokepoint.adapters`); `applies_to()` only attempts the optional `mcp`
import lazily, inside the method body, so `import chokepoint` never needs it.
...
from chokepoint.errors import AdapterError, ConfigurationError
...
_GUARDED = "__chokepoint_mcp_guarded__"
...
def guard_mcp_session(session: Any, interceptor: ChokepointInterceptor) -> Any:
    ...
    call raises `GuardBlocked` and never reaches the server.
        ...
        async with ClientSession(read, write) as session:
            ...
            chokepoint.wrap(session, interceptor)
...
def _guard_method_keyed(low_level: Any, interceptor: ChokepointInterceptor) -> None:
...
def _guard_type_keyed(low_level: Any, interceptor: ChokepointInterceptor) -> None:
...
def guard_mcp_server(server: Any, interceptor: ChokepointInterceptor) -> Any:
    ...
    server's tools are defined:
        ...
        chokepoint.wrap(server, interceptor)
