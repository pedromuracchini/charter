...
`EscalationHandler` implementation under `chokepoint.escalation`.

Arguments are redacted before they go out. An escalation message is the least
controlled destination Chokepoint writes to — it lands in a Slack channel with
its own membership, or at a webhook endpoint on someone else's infrastructure
— so it gets the same scrubbing the ledger does. See `chokepoint.redaction`.
...
from chokepoint.decisions import RuleResult
from chokepoint.redaction import current_redactor
