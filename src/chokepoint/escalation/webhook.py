...
`chokepoint.escalation.slack`). Uses only `urllib.request` — no new dependency.
"""
...
from chokepoint.core.escalation import EscalationHandler
from chokepoint.decisions import RuleResult

logger = logging.getLogger("chokepoint.escalation.webhook")
...
class WebhookEscalationHandler(EscalationHandler):
    ...
    secret, a bearer token, ...) — per `SECURITY.md`, Chokepoint has no opinion
