...
at import time, or via `chokepoint lint`), not on every request. It operates on
...
from chokepoint.core.policy_set import Policy, PolicySet
...
#: Kept so `from chokepoint.linter import Severity` still resolves. Renamed
#: because `chokepoint.Severity` is a different Literal entirely
...
class LintFinding:
    ...
    `severity="error"` is what `chokepoint lint` exits non-zero on, so it is
...
def _check_scoped_policy_without_registry(
    policies: list[Policy], registry: ChokepointRegistry | None
) -> list[LintFinding]:
            ...
            findings.append(
                LintFinding(
                    ...
                    message=(
                        ...
                        "registered in any ChokepointRegistry — caller_role will always be None"
                    ),
                    policy_name=policy.name,
                )
            )
