"""Chokepoint's exception and warning hierarchy.

Every error the framework raises on purpose derives from `ChokepointError`, so a
...
Warnings are separate. Chokepoint warns rather than raises for configurations
...
class ChokepointError(Exception):
    """Base class for every error Chokepoint raises deliberately."""


class ConfigurationError(ChokepointError, ValueError):
...
class EscalationError(ChokepointError, RuntimeError):
...
class LedgerError(ChokepointError):
...
class AdapterError(ChokepointError, TypeError):
    """No adapter could wrap the given agent or tool object."""


class ChokepointWarning(UserWarning):
    """Base class for every warning Chokepoint issues."""


class ConfigurationWarning(ChokepointWarning):
