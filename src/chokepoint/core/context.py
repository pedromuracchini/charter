...
Built fresh for each tool call by the evaluation engine (`chokepoint._engine`),
combining the tool's own arguments with the ambient session/identity data carried
by `chokepoint._scope.ExecutionScope`. Pydantic gives IDE autocomplete and type
...
class GuardContext(BaseModel):
    ...
    def build(
        ...
        *,
        ...
        ambient `ExecutionScope` (set by the interceptor or `chokepoint.session()`)."""
        ctx = cls(
            tool_name=tool_name,
            args=args,
            result=result,
    ...
    def calls_this_session(self, tool_name: str | None = None) -> int:
        ...
        `chokepoint.state.ALL_TOOLS` (`"*"`) for every tool combined. Counts
    ...
    def record_spend(self, key: str, amount: float) -> float:
        ...
        whether a call may proceed. See `chokepoint.policies.limits.budget_policy`.
        """
        if self._call_state is None:
            return 0.0
