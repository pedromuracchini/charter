...
from chokepoint._scope import ExecutionScope, use_scope
from chokepoint.core.decorator import _is_async_tool
...
from chokepoint.redaction import Redactor
...
def _warn_if_shadowed(
    ...
    warnings.warn(
        f"{names} was consumed by ChokepointInterceptor.call() as an interceptor option, "
...
class ChokepointInterceptor:
    ...
    Args:
        ...
        agent_id: Who this interceptor speaks for. Omitting it leaves
            ...
            `AgentScopedPolicy` with `allowed_roles` — `chokepoint lint` reports
            this as an error.
    ...
    def __init__(
        ...
        mode: Mode = "enforce",
        *,
        registry: ChokepointRegistry | None = None,
    ...
    def wrap_tool(self, tool_name: str, func: Callable[..., Any] | ReversibleAction) -> Callable[..., Any]:
        ...
        wrapped.__chokepoint_tool_name__ = tool_name  # type: ignore[union-attr]
    ...
    def use(self, agent: Any) -> Any:
        ...
        """
        from chokepoint.adapters.base import wrap as _wrap
