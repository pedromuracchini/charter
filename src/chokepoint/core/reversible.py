...
`chokepoint lint` warns about a `"high"` action with no `escalate_to`.
"""
...
from chokepoint.core.escalation import validate_escalate_to
from chokepoint.decisions import BLOCK, ESCALATE, RuleResult
...
class ReversibleAction:
    ...
    `ChokepointInterceptor.acall()` detect an async `do_fn` automatically (see
    ...
    `ChokepointInterceptor.call()`) returns an un-awaited coroutine, which is a
