"""`ChokepointRegistry` — central registry of agent identities and roles.

Used by `ChokepointInterceptor` to populate `ctx.caller_role` / `ctx.trust_level`
...
class ChokepointRegistry:
    """Maps `agent_id -> AgentIdentity`.

    One registry shared by several `ChokepointInterceptor`s is the alternative
    ...
    def __repr__(self) -> str:
        ...
        return f"<ChokepointRegistry agents={len(self._identities)}[{agents}]>"
    ...
    def register(
        ...
        Args:
            ...
            delegation_chain: This agent's **ancestors only**, ordered
                ...
                path — see `ChokepointInterceptor._self_inclusive_chain`.
