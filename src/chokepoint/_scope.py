...
from somewhere else. `ExecutionScope` is that somewhere: `ChokepointInterceptor` (or
...
class ExecutionScope:
    ...
    #: see zero rather than stale numbers. See `chokepoint.state`.
    call_state: CallState | None = None


_ROOT_SCOPE = ExecutionScope()
_current_scope: ContextVar[ExecutionScope] = ContextVar("chokepoint_scope", default=_ROOT_SCOPE)
...
def use_scope(scope: ExecutionScope) -> Iterator[ExecutionScope]:
    ...
    Used by `ChokepointInterceptor` so that any `@guard`-decorated helper called
...
def session(**overrides: Any) -> Iterator[ExecutionScope]:
    ...
    `ChokepointInterceptor` — the interceptor manages scope on its own.
    """
